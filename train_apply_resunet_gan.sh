...
databasePath="database_v6_resnet.db"

hash=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1)
echo "Output Hash: $hash"

trainFolder="/lustre_files/data_folder/train/" 
trainPaths=()
for file in ${trainFolder}*cleaned.nc; do     trainPaths+=("$file"); done
trainPathsString="${trainPaths[@]}"
# #test path: 
# trainPathsString="/home/jovyan/data_folder/train/train_230_cleaned.nc"

testFolder="/lustre_files/data_folder/test/" 
testPaths=()
for file in ${testFolder}*cleaned.nc; do     testPaths+=("$file"); done
testPathsString="${testPaths[@]}"
# #test path: 
# testPathsString="/home/jovyan/data_folder/test/test_202_cleaned.nc"
